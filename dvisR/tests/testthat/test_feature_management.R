...
## grab_dependency_functions is correct

test_that("grab_dependency_functions works", {
  res <- grab_dependency_functions()
  
  expect_true(is.list(res))
  expect_true(all(sapply(res, is.function)))
})

test_that("grab_dependency_functions yields functions that work", {
  set.seed(10)
  res <- grab_dependency_functions()
...
test_that(".apply_feature_list works", {
  ...
  res <- .apply_feature_list(dat_2col, feature_list = feature_list)
...
test_that(".apply_feature_list has meaningful tryCatch", {
  ...
  res <- .apply_feature_list(dat_2col, NA, feature_list = feature_list)
...
test_that(".initialize_feature_matrix works", {
  ...
  template_vec <- rep(0, 3)
  names(template_vec) <- c("Feature 1", "Feature 2", "Feature 3)")
  
  res <- .initialize_feature_matrix(template_vec, ntrials, new_pairs_per_round, minimum_instances_first_phase)
  
  expect_true(all(is.na(res)))
  expect_true(all(colnames(res) == names(template_vec)))
...
test_that(".expand_feature_matrix works", {
  ...
  feature_mat <- .initialize_feature_matrix(template_vec, ntrials, new_pairs_per_round, minimum_instances_first_phase)
