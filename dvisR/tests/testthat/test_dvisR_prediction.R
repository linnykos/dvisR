...
test_that(".compute_remaining_feature_mat works with clusters", {
  set.seed(10)
  n <- 50; p <- 50
  dat <- MASS::mvrnorm(n = n, mu = rep(0, p), Sigma = diag(p))
  cluster_labels <- sample(c(1,2,3), 50, replace = T)
  obj <- dvisR_system(dat, cluster_labels = cluster_labels,
  ...
  expect_true(is.matrix(res$feature_mat))
  expect_true(ncol(res$pairs_mat) == 2)
  expect_true(ncol(res$feature_mat) == ncol(obj$df)-4)
  expect_true(nrow(res$pairs_mat) == p*(p-1)/2 - nrow(obj$df))
})
...
test_that("dvisR_prediction works", {
 ...
 expect_true(all(sort(names(res)) == sort(c("df", "probability", "dim"))))
...
test_that("dvisR_prediction works with clusters", {
  set.seed(10)
  n <- 50; p <- 50
  dat <- MASS::mvrnorm(n = n, mu = rep(0, p), Sigma = diag(p))
  cluster_labels <- sample(c(1,2,3), 50, replace = T)
